...
class JSONReaderDbForFirstPipeline() extends CasCollectionReader_ImplBase {
    ...
    //@ConfigurationParameter(name = JSONReaderDbForFirstPipeline.USER_NAME, defaultValue = Array(""))
    val userName: String = ConfigFactory.load().getString("app.user")

    //@ConfigurationParameter(name = JSONReaderDbForFirstPipeline.PW, defaultValue = Array(""))
    val pw: String = ConfigFactory.load().getString("app.pw")

    //@ConfigurationParameter(name = JSONReaderDbForFirstPipeline.SERVER_ADDRESS, defaultValue = Array(""))
    val serverAddress: String = ConfigFactory.load().getString("app.server")

    //@ConfigurationParameter(name = JSONReaderDbForFirstPipeline.PORT, defaultValue = Array(""))
    val port: String = ConfigFactory.load().getString("app.port")

    //@ConfigurationParameter(name = JSONReaderDbForFirstPipeline.DB, defaultValue = Array(""))
    val db: String = ConfigFactory.load().getString("app.db")

    //@ConfigurationParameter(name = JSONReaderDbForFirstPipeline.COLLECTION_NAME, defaultValue = Array(""))
    val collectionName: String = ConfigFactory.load().getString("app.collection")

    //@ConfigurationParameter(name = JSONReaderDbForFirstPipeline.FILE_LOCATION, defaultValue = Array(""))
...
/*object JSONReaderDbForFirstPipeline {
 ...
 // val config = ConfigFactory.parseFile(new File(configPath))
  ...
  final val SERVER_ADDRESS = "server"
  final val PORT = "port"
  final val DB = "db"
  final val COLLECTION_NAME = "collectionName"
  final val FILE_LOCATION = "lastCrawlTimeFile"
}*/

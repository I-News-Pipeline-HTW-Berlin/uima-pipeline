...
class ReadingTimeEstimator extends JCasAnnotator_ImplBase{
  ...
  def estimateReadingTime(wordCount: Int, wordsPerMinute: Double = 200.0) : Int = {
    val estimateTime: Double = wordCount.toDouble / wordsPerMinute
    val minutes: Int = estimateTime.toInt
    val seconds: Double = (estimateTime - minutes) * 0.60
    seconds match {
  ...
  /**
   * Gets the number of words of document text, calculates reading time and uses MetaDataStringField annotation to
   * save reading time for document.
   * @param aJCas
   */
  override def process(aJCas: JCas): Unit = {
    val numOfWords: Int = JCasUtil.select(aJCas, classOf[Token]).size()
    val readingTime: Int = estimateReadingTime(numOfWords, wordsPerMinute.toDouble)
    val metaDataStringField: MetaDataStringField = new MetaDataStringField(aJCas, 0, numOfWords-1)
